    ...
    >
      <template v-slot:items="props">
        ...
        <td class="text-xs-right">
          <v-btn
            ...
            v-bind:to="{name: 'facultyMajor', params: {facultyID: props.item.id}}"
          >
            <v-icon>notes</v-icon>
          </v-btn>
        </td>
        <td class="text-xs-right">
          ...
          v-bind:to="{name: 'facultyDetail', params: {facultyID: props.item.id}}">
          ...
          <v-btn
            ...
            icon
            color="pink"
            @click="(selectedData.id = props.item.id,selectedData.name = props.item.faculty_name),dialogDel() "
          >
      ...
      <template v-slot:footer>
        <td :colspan="headers.length">
          <strong>จำนวนคณะทั้งหมด {{data.length}} คณะ</strong>
        </td>
      </template>
    </v-data-table>
  </v-app>
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: "รายชื่อคณะ", value: "faculty_name" },

        { text: "สาขาวิชา", align: "right", sortable: false },
  ...
  methods: {
    ...
    deleteData: function() {
      ...
      let apiURL = "http://localhost:1337/faculty/delete/";
